// File: src/components/PickersManager.jsx
...
const PickersManager = ({ open, onClose }) => {
  ...
  useEffect(() => {
    if (open) {
      const fetchItems = async () => {
        setLoading(true);
        try {
          const data = await kdb.run({ module: "OrderRouting", name: "managePickersGet", data: {} });
          setItems(data.items);
        } catch (error) {
          console.error("שגיאה בטעינת מלקטים:", error);
        } finally {
          setLoading(false);
        }
      };
  ...
  const handleSave = async () => {
    try {
      await kdb.run({
        module: "OrderRouting",
        name: editId ? "managePickersUpdate" : "managePickersCreate",
      ...
      console.error("שגיאה בשמירת מלקט:", error);
    }
  };

  const handleDelete = async (id) => {
    if (window.confirm('האם למחוק את המלקט?')) {
      try {
        await kdb.run({ module: "OrderRouting", name: "managePickersDelete", data: { id } });
        const data = await kdb.run({ module: "OrderRouting", name: "managePickersGet", data: {} });
        setItems(data.items);
      } catch (error) {
        console.error("שגיאה במחיקת מלקט:", error);
      }
    }
  };
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth>
      <DialogTitle>ניהול מלקטים</DialogTitle>
      <DialogContent>
        <Button variant="contained" color="primary" onClick={() => handleOpenForm()} className="mb-4">הוסף מלקט</Button>
        {loading ? (
          <div>טוען...</div>
        ) : (
