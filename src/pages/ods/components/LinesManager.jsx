// File: src/components/LinesManager.jsx
...
const LinesManager = ({ open, onClose }) => {
  ...
  useEffect(() => {
    if (open) {
      const fetchItems = async () => {
        setLoading(true);
        try {
          const data = await kdb.run({ module: "OrderRouting", name: "manageLinesGet", data: {} });
          setItems(data.items);
        } catch (error) {
          console.error("שגיאה בטעינת קווים:", error);
        } finally {
          setLoading(false);
        }
      };
  ...
  const handleSave = async () => {
    try {
      await kdb.run({
        module: "OrderRouting",
        name: editId ? "manageLinesUpdate" : "manageLinesCreate",
      ...
      console.error("שגיאה בשמירת קו:", error);
    }
  };

  const handleDelete = async (id) => {
    if (window.confirm('האם למחוק את הקו?')) {
      try {
        await kdb.run({ module: "OrderRouting", name: "manageLinesDelete", data: { id } });
        const data = await kdb.run({ module: "OrderRouting", name: "manageLinesGet", data: {} });
        setItems(data.items);
      } catch (error) {
        console.error("שגיאה במחיקת קו:", error);
      }
    }
  };
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth>
      <DialogTitle>ניהול קווים</DialogTitle>
      <DialogContent>
        <Button variant="contained" color="primary" onClick={() => handleOpenForm()} className="mb-4">הוסף קו</Button>
        {loading ? (
          <div>טוען...</div>
        ) : (
