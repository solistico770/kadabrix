// File: src/components/DriversManager.jsx
...
const DriversManager = ({ open, onClose }) => {
  ...
  useEffect(() => {
    if (open) {
      const fetchItems = async () => {
        setLoading(true);
        try {
          const data = await kdb.run({ module: "OrderRouting", name: "manageDriversGet", data: {} });
          setItems(data.items);
        } catch (error) {
  ...
  const handleSave = async () => {
    try {
      await kdb.run({
        module: "OrderRouting",
        name: editId ? "manageDriversUpdate" : "manageDriversCreate",
      ...
      console.error("שגיאה בשמירת נהג:", error);
    }
  };

  const handleDelete = async (id) => {
    if (window.confirm('האם למחוק את הנהג?')) {
      try {
        await kdb.run({ module: "OrderRouting", name: "manageDriversDelete", data: { id } });
        const data = await kdb.run({ module: "OrderRouting", name: "manageDriversGet", data: {} });
        setItems(data.items);
      } catch (error) {
        console.error("שגיאה במחיקת נהג:", error);
      }
    }
  };
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth>
      <DialogTitle>ניהול נהגים</DialogTitle>
      <DialogContent>
        <Button variant="contained" color="primary" onClick={() => handleOpenForm()} className="mb-4">הוסף נהג</Button>
        {loading ? (
          <div>טוען...</div>
        ) : (
